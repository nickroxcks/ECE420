...
void gauss_elim(){
    ...
    for (int cur_row = 0;cur_row<num_rows-1;cur_row++){
        ...
        #pragma omp parallel for num_threads(thread_count) default(none) private(i,j,temp) shared(Matrix,num_columns,num_rows,cur_row)
...
void jordan_elim(){
    int cur_row;
    int row_section;
    #pragma omp parallel for num_threads(thread_count) default(none) private(cur_row,row_section) shared(Matrix,num_columns,num_rows)
