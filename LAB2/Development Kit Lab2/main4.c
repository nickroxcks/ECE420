//read write locks for each element
#include <stdio.h>
#include <stdlib.h>
#include <sys/types.h>
...
pthread_rwlock_t *read_write_locks;;
...
void* client_side(void *descriptor){
    ...
    if(request.is_read){
        pthread_rwlock_rdlock(&read_write_locks[request.pos]);
        getContent(buffer_out,request.pos,server_array);
        pthread_rwlock_unlock(&read_write_locks[request.pos]);
    }

    else{
        pthread_rwlock_wrlock(&read_write_locks[request.pos]);
...
int main(int argc, char* argv[])
{
    ...
    read_write_locks = malloc(array_size * sizeof(pthread_rwlock_t));
    for(j=0;j<array_size;j++){
        pthread_rwlock_init(&read_write_locks[j],NULL);
    }
